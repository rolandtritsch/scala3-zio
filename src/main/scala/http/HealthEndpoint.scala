...
/** Basic health check endpoint for liveness probes.
  *
  * This endpoint always returns HTTP 200 OK when the server is running.
  * It performs no external checks and is intended for Kubernetes liveness
  * probes or load balancer health checks that only need to verify the
  * process is alive.
  *
  * '''Endpoint:''' GET /health
  *
  ...
  *
  * @see [[HealthDeepEndpoint]] for comprehensive health checks
  */
